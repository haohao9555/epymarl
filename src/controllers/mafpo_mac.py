...
# 释）。self._last_x1_raw 存的是 sigmoid 之前的无界积分终点，供
# compute_initial_cfm_loss / parallel_runner.py 的 collect_action_raw 读取，
# 让 CFM 回归的插值目标是无界 latent，不是 sigmoid 之后被压缩过的 action。
#-----------------------------
...
class MAFPOMAC:
    ...
    def compute_initial_cfm_loss(
        ...
        with th.no_grad():
            ...
            # 改回纯 MSE：Huber 会改变 ℓ 本身的数值含义（不再是 ELBO 意义下的负对数
            # 似然代理），破坏 exp(clamp(Delta ell)) 的 ELBO 解释。稳定性改用
            # actor loss 里的 anchor 惩罚项，见 fpopp_learner.py。
