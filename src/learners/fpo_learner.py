...
class PPOLearner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        for k in range(self.args.epochs):
            ...
            approx_kl = ((old_log_pi_taken.detach() - log_pi_taken) * mask).sum() / mask.sum()
            clip_fraction = (
                (((ratios > 1 + self.args.eps_clip) | (ratios < 1 - self.args.eps_clip)).float() * mask).sum()
                / mask.sum()
            )
            entropy_mean = (entropy * mask).sum() / mask.sum()
            ratio_mean = (ratios * mask).sum() / mask.sum()
            ratio_std = th.sqrt((((ratios - ratio_mean) * mask) ** 2).sum() / mask.sum())
            pi_taken_mean = (pi_taken * mask).sum() / mask.sum()
            old_pi_taken_mean = (old_pi_taken * mask).sum() / mask.sum()
            advantage_mean = (advantages * mask).sum() / mask.sum()
            advantage_std = th.sqrt((((advantages - advantage_mean) * mask) ** 2).sum() / mask.sum())
    ...
    #---------------新增：CFM loss 计算接口------------------------------
    def compute_cfm_loss(self, batch: EpisodeBatch, mac=None):
        mac = self.mac if mac is None else mac

        # Step 1: Read rollout actions x_1 from the episode batch; do not resample actions during training.
        action = batch["actions"][:, :-1].float()

        # Step 2: Read the rollout-time CFM noise eps and interpolation time t from the same episode batch.
        eps = batch["cfm_eps"][:, :-1]
        cfm_t = batch["cfm_t"][:, :-1]

        # Step 3: Add the CFM sample dimension to actions so each rollout action is paired with N eps/t samples.
        action = action.unsqueeze(3).expand_as(eps)

        # Step 4: Build the interpolated point x_t = t * eps + (1 - t) * action.
        x_t = cfm_t * eps + (1 - cfm_t) * action

        # Step 5: Align observations with the N CFM samples for each batch/time/agent point.
        obs = batch["obs"][:, :-1]
        obs = obs.unsqueeze(3).expand(-1, -1, -1, eps.size(3), -1)

        # Step 6: Flatten batch/time/agent/sample dimensions before feeding obs, x_t, and t into flow policy.
        flat_obs = obs.reshape(-1, obs.shape[-1])
        flat_x_t = x_t.reshape(-1, x_t.shape[-1])
        flat_t = cfm_t.reshape(-1, 1)

        # Step 7: Ask the flow policy to predict either velocity or eps from the fixed rollout CFM points.
        flow_policy = self._get_flow_policy(mac)
        pred = flow_policy(flat_obs, flat_x_t, flat_t)
        pred = pred.reshape_as(eps)

        # Step 8: Select the CFM regression target. For velocity target, dx_t/dt = eps - action.
        ...
        if cfm_target_type == "velocity":
            ...
            raise ValueError(
                "Unsupported cfm_target_type '{}'. Use 'velocity' or 'eps'.".format(
                    cfm_target_type
                )
            )

        # Step 9: Compute per-sample MSE over the action dimension and keep [bs, t, agents, N, 1].
        cfm_loss = ((pred - target) ** 2).mean(dim=-1, keepdim=True)
        return cfm_loss

    def _get_flow_policy(self, mac):
        if hasattr(mac.agent, "flow_policy"):
            return mac.agent.flow_policy
        if hasattr(mac.agent, "flow_forward"):
            return mac.agent.flow_forward
        raise AttributeError(
            "FPO compute_cfm_loss expects mac.agent.flow_policy(obs, x_t, t) "
            "or mac.agent.flow_forward(obs, x_t, t), but neither exists yet."
        )
    #----------------------
